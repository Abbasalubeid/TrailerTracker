...
function discoverMovies(page, genre = 0, sortBy = "") {
  const sortByParam = sortBy ? `&sort_by=${sortBy}` : "";
  const url = genre === 0
    ? `${BASE_URL}discover/movie?api_key=${API_KEY}&page=${page}${sortByParam}`
    : `${BASE_URL}discover/movie?api_key=${API_KEY}&page=${page}&with_genres=${genre}${sortByParam}`;
